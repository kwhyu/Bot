...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_menu_selection:
        if user_id in user_menu_selection and user_menu_selection[user_id] == 'cari_mhs':
            await search_and_reply_mhs(update, context, text)
            del user_menu_selection[user_id]
        elif user_id in user_menu_selection and user_menu_selection[user_id] == 'cari_matkul':
...
async def search_and_reply_mhs(update: Update, context: ContextTypes.DEFAULT_TYPE, query: str):
...
async def search_matkul(query: str) -> str:
    # Query the database to search for a student by ID or name
    connection = create_connection()
    cursor = connection.cursor(dictionary=True)
...
async def search_and_reply_matkul(update: Update, context: ContextTypes.DEFAULT_TYPE, query: str):
    result = await search_matkul(query)
    if result:
...
async def handle_text_message_matkul(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.message.text.strip()
    await search_and_reply_matkul(update, context, query)
...
if __name__ == '__main__':
    ...
    # Menambahkan handler pesan teks
    app.add_handler(MessageHandler(filters.TEXT, handle_text_message_mhs))
    app.add_handler(MessageHandler(filters.TEXT, handle_text_message_matkul))
